#!/usr/bin/env python3
"""Emit RESULTS.md from metrics.json (femtovg vs browsers) and
metrics_selfref.json (same-engine test-vs-ref under WPT's fuzzy rule)."""
import json, os

W = os.path.dirname(os.path.abspath(__file__))
rows = json.load(open(f"{W}/metrics.json"))
selfref = json.load(open(f"{W}/metrics_selfref.json"))
man = json.load(open(f"{W}/manifest.json"))

VERDICT = {
    "clip-path-clip-nested-twice": "match (nested clipPath chain via clip_path intersection)",
    "mask-and-nested-clip-path": "match (nested clipPath chain)",
    "mask-negative-scale": "match",
    "mask-nested-clip-path-001": "match (clip coverage, two levels)",
    "mask-nested-clip-path-002": "DEVIATION: nested layer mask origin (finding A)",
    "mask-nested-clip-path-003": "DEVIATION: nested layer mask origin (finding A)",
    "mask-nested-clip-path-004": "match (clip coverage)",
    "mask-nested-clip-path-005": "match (clip coverage)",
    "mask-nested-clip-path-006": "match (clip coverage)",
    "mask-nested-clip-path-007": "match (clip coverage)",
    "mask-nested-clip-path-008": "match (clip coverage)",
    "mask-nested-clip-path-009": "match (clip coverage)",
    "mask-nested-clip-path-010": "match",
    "mask-nested-clip-path-panning-001": "DEVIATION: nested layer mask origin (finding A)",
    "mask-nested-clip-path-panning-002": "DEVIATION: nested layer mask origin (finding A)",
    "mask-objectboundingbox-content-clip-transform": "edge only: hard clip edge vs AA mask edge (finding B)",
    "mask-objectboundingbox-content-clip": "edge only: hard clip edge (finding B)",
    "mask-on-thin-stroked-path-userspaceonuse": "match",
    "mask-text-001": "within browser envelope (fallback font on both sides, finding D)",
    "mask-userspaceonuse-content-clip-transform": "edge only: hard clip edge (finding B)",
    "mask-userspaceonuse-content-clip": "edge only: hard clip edge (finding B)",
    "mask-with-filter": "match",
    "mask-text-001-arial": "within browser envelope (same face both sides)",
}

out = []
out.append("# WPT css-masking candidates: femtovg (#323 layer masks + #324 clip paths) vs Chromium 131 and Firefox\n")
out.append("""Measured 2026-09-16 on the `corpus-all3` harness tree (`/private/tmp/wt-all3`, the whole
...
for r in rows:
    n, z = r["case"], r["zoom"]
    t, rf, tr = r["fvg_vs_chr_test"], r["fvg_vs_chr_ref"], r["chr_test_vs_chr_ref"]
    s = selfref[f"{n}@{z}"]
    ff = f"{r['chr_test_vs_ff_test']['box_over20_pct']:.2f}% / {r['chr_test_vs_ff_test']['box_struct_pct']:.3f}%" if z == "1.0" else "-"
    fv = f"{r['fvg_vs_ff_test']['box_over20_pct']:.2f}% / {r['fvg_vs_ff_test']['box_struct_pct']:.3f}%" if z == "1.0" else "-"
    a, c = s["fvg_test_vs_fvg_ref"], s["chr_test_vs_chr_ref"]
    out.append(
        f"| {n} | {z} | {t['box_over20_pct']:.2f}% / {t['box_struct_pct']:.3f}% / {t['max']} / {t['box_exact']} "
        f"| {rf['box_over20_pct']:.2f}% / {rf['box_struct_pct']:.3f}% / {rf['max']} "
        f"| {tr['box_over20_pct']:.2f}% | {ff} | {fv} "
        f"| {a['exact']} / {a['max']} -> {'pass' if a['pass'] else 'FAIL'} (fuzzy {s['fuzzy']}) "
        f"| {c['exact']} / {c['max']} -> {'pass' if c['pass'] else 'FAIL'} "
        f"| {VERDICT[n]} |"
    )
...
""")
open(f"{W}/RESULTS.md", "w").write("\n".join(out))
print("RESULTS.md written", len(rows), "rows")
