    ...
    T: Renderer,
{
    ...
    fn draw_text(
        ...
        // The baseline of a horizontal run is shared by every glyph (it is
        // `cursor_y + alignment_offset` in `layout`). Capture it from the first
        // drawable glyph in scaled shaping space; decorations hang off it below.
        let baseline_scaled = layout
            .glyphs
            .iter()
            .find(|shaped_glyph| !shaped_glyph.c.is_control())
            .map(|shaped_glyph| shaped_glyph.y);
