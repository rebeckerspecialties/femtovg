...
fn capture_mask(
    ...
    let image = canvas
        ...
        .ok()?;
    HARNESS_IMAGES.lock().unwrap().push(image);
...
fn draw_filtered(
    ...
    HARNESS_IMAGES.lock().unwrap().push(noise);
...
/// Images the harness itself creates during a frame (mask captures,
/// turbulence noise), deleted after the frame's flush so they neither leak
/// across frames nor count as pool creations in the FRAME log.
static HARNESS_IMAGES: std::sync::Mutex<Vec<femtovg::ImageId>> = std::sync::Mutex::new(Vec::new());
...
fn main() {
    ...
    // ZOOM_SWEEP="lo:hi:n": n frames whose zoom steps lo -> hi and back (a
    // triangle over the n frames, hi at the midpoint), each frame
    // re-rendering the whole scene at its own zoom; the command-line zoom
    // is then ignored. Without it every frame renders at the command-line
    // zoom. Either mode prints one FRAME line per frame under FRAME_LOG=1
    // (ZOOM_SWEEP implies it).
    let zooms: Vec<f32> = match std::env::var("ZOOM_SWEEP") {
        Ok(spec) => {
            let mut it = spec.split(':');
            let lo: f32 = it.next().unwrap().parse().expect("ZOOM_SWEEP lo");
            let hi: f32 = it.next().unwrap().parse().expect("ZOOM_SWEEP hi");
            let n: usize = it.next().unwrap().parse().expect("ZOOM_SWEEP n");
            (0..n)
                .map(|i| {
        ...
        Err(_) => vec![scale; frames],
    };
    let frame_log = std::env::var("FRAME_LOG").is_ok() || std::env::var("ZOOM_SWEEP").is_ok();
    // Live wgpu textures (wgpu-core's registry): the delta across a frame's
    // recording counts every texture the library allocated in it - the
    // transient pool's creations plus, in the first frame, gradient textures
    // - with the harness's own mask and noise images subtracted.
    let live_textures = |instance: &wgpu::Instance| -> usize {
        instance
            .generate_report()
            .map(|r| r.hub.textures.num_kept_from_user)
            .unwrap_or(0)
    };
    let mut textures_baseline = live_textures(&instance);
    for (frame, &scale) in zooms.iter().enumerate() {
        let started = std::time::Instant::now();
        let layers_before = LAYERS.load(std::sync::atomic::Ordering::Relaxed);
        let pass_before = PASS_THROUGH.load(std::sync::atomic::Ordering::Relaxed);
        ...
        let bytes_at_flush = canvas.transient_image_bytes();
        TRANSIENT_AT_FLUSH.store(bytes_at_flush, std::sync::atomic::Ordering::Relaxed);
        let textures_before_flush = live_textures(&instance);
        let record_ms = started.elapsed().as_secs_f64() * 1e3;
        let commands = canvas.flush_to_output(&target);
        queue.submit(commands);
        device.poll(wgpu::PollType::wait_indefinitely()).unwrap();
        let wall_ms = started.elapsed().as_secs_f64() * 1e3;
        let bytes_after_flush = canvas.transient_image_bytes();
        let harness_images: Vec<femtovg::ImageId> = std::mem::take(&mut *HARNESS_IMAGES.lock().unwrap());
        let harness_created = harness_images.len();
        for id in harness_images {
            canvas.delete_image(id);
        }
        let textures_after_flush = live_textures(&instance);
        if frame_log {
            eprintln!(
                "FRAME {frame} zoom={scale:.4} wall_ms={wall_ms:.3} record_ms={record_ms:.3} \
                 bytes_at_flush={bytes_at_flush} bytes_after_flush={bytes_after_flush} \
                 created={} textures_before_flush={textures_before_flush} \
                 textures_after_flush={textures_after_flush} harness_images={harness_created} \
                 layers={} pass_through={}",
                textures_before_flush as isize - textures_baseline as isize - harness_created as isize,
                LAYERS.load(std::sync::atomic::Ordering::Relaxed) - layers_before,
                PASS_THROUGH.load(std::sync::atomic::Ordering::Relaxed) - pass_before,
            );
        }
        textures_baseline = textures_after_flush;
    }
