...
ap.add_argument('--chromium-only', action='store_true', help='render only the Chromium pair (no binary needed)')
args = ap.parse_args()
# Chromium gets file:// URLs, which must be absolute: a relative --out silently loads a blank page.
args.out = os.path.abspath(args.out); args.suite = os.path.abspath(args.suite)
# The framing the harness and make_ref.py take from the environment; the window must match it.
WINDOW = f"{os.environ.get('FRAME_W', '460')},{os.environ.get('FRAME_H', '260')}"
...
def chromium(svg, out):
    ...
    for attempt in range(3):
        # A profile of its own per launch: back-to-back launches sharing the default one come back blank.
        with tempfile.TemporaryDirectory() as profile:
            subprocess.run([CHR, '--headless', '--disable-gpu', f'--user-data-dir={profile}', f'--screenshot={out}',
                            f'--window-size={WINDOW}', '--default-background-color=FFFFFFFF', 'file://' + html],
                           capture_output=True)
        ...
        time.sleep(1)
    print(f'chromium kept returning blank screenshots for {svg}; using the last one', file=sys.stderr)
    return px
