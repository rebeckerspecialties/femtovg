...
fn capture_mask(
    ...
    draw_nodes(canvas, mask.root().children(), scale);
...
fn group_effects(
    ...
    let mask = group.mask().and_then(|m| capture_mask(canvas, m, canvas_w, canvas_h, scale));
...
fn draw_nodes(canvas: &mut Canvas<WGPURenderer>, children: &[usvg::Node], scale: f32) {
    ...
    for node in children {
        match node {
            usvg::Node::Group(group) => {
                    ...
                    for node in clip.root().children() {
                        if let usvg::Node::Path(p) = node {
                            ...
                            for seg in p.data().segments() {
                                match seg {
                                    PathSegment::MoveTo(pt) => combined.move_to(pt.x, pt.y),
                                    PathSegment::LineTo(pt) => combined.line_to(pt.x, pt.y),
                                    PathSegment::CubicTo(a, b, pt) => {
                                        combined.bezier_to(a.x, a.y, b.x, b.y, pt.x, pt.y)
                                    }
                                    PathSegment::QuadTo(a, pt) => combined.quad_to(a.x, a.y, pt.x, pt.y),
                ...
                match group_effects(canvas, group, W as usize, H as usize, scale) {
                    Some(fx) => {
                        canvas.begin_layer(&fx);
                        draw_nodes(canvas, group.children(), scale);
                        canvas.end_layer();
                    }
                    None => draw_nodes(canvas, group.children(), scale),
                }
                if clipped {
                    canvas.restore();
                }
...
fn main() {
    ...
    draw_nodes(&mut canvas, tree.root().children(), scale * fit);
